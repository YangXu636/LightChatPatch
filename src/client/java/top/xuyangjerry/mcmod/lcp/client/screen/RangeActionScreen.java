...
public class RangeActionScreen extends Screen {
    ...
    protected void init() {
        ...
        Button cancelBtn = Button.builder(CommonComponents.GUI_CANCEL, btn -> this.minecraft.gui.setScreen(parent))
                .bounds(centerX - BUTTON_WIDTH / 2, y, BUTTON_WIDTH, BUTTON_HEIGHT)
                .build();
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float partialTick) {
        ...
        for (int i = 0; i < buttons.size(); i++) {
            ...
            if (i == selectedButton && !btn.isHovered()) {
                ...
                int w = btn.getWidth();
                int h = btn.getHeight();
                g.fill(btnX - 1, btnY - 1, btnX + w + 1, btnY, 0xFFFFFFFF);
                g.fill(btnX - 1, btnY + h, btnX + w + 1, btnY + h + 1, 0xFFFFFFFF);
                g.fill(btnX - 1, btnY, btnX, btnY + h, 0xFFFFFFFF);
                g.fill(btnX + w, btnY, btnX + w + 1, btnY + h, 0xFFFFFFFF);
            }
        }
    }
